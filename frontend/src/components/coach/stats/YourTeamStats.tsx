...
const YourTeamStats = () => {
  ...
  const { data: players } = usePlayersFromTeamQuery(user?.slug_team || "");
      ...
      <section className="space-y-6">
        <div className="bg-white p-6 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col lg:flex-row justify-between items-center gap-6">
          <div className="space-y-1 text-center lg:text-left">
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Balance Colectivo</p>
          </div>
        ...
        {isForLoading || isAgainstLoading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <HeatMap stats={statsForTeam || []} mode="for" title={`Aciertos - ${actionType}`} />
            <HeatMap stats={statsAgainstTeam || []} mode="against" title={`Errores - ${actionType}`} />
          </div>
        )}
      </section>
